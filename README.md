...
- The pre-ingested graph includes CVE, CWE, CAPEC, ATT&CK, NICE, and DCWF datasets with crosswalks. Embeddings are not included -- only the Cypher-based ask path is supported. To enable RAG-based search, run `docker compose run --rm app setup embeddings` after the stack is up (requires OpenAI API credits and takes some time). This only needs to be done once.
